...
prek run --all-files                           # ruff, gitleaks, bandit, semgrep (python + actions), osv-scanner, unit tests — scanners from their pinned rev in prek's cache; severity gates match CI (bandit HIGH, semgrep ERROR)
...
- Tool versions pinned in two places that must match: `security.yml` (semgrep image tag, `BANDIT_VERSION`, `GITLEAKS_VERSION`) / `.github/actions/osv-image-scan/action.yml` (`OSV_SCANNER_VERSION`) and the `rev` of the corresponding hook in `.pre-commit-config.yaml`. Dependabot does not bump hook `rev`s — use `prek auto-update` when bumping CI.
