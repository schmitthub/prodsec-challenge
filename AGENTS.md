...
`requirements.txt` is the canonical dependency manifest — SBOM and all SCA (osv, dependency-review) read only it. `pyproject.toml` + `uv.lock` mirror the same pins for uv-based dev/CI (`dev` group is ruff only) and must stay in sync (CI checks this). Security scanners are deliberately **not** uv deps — they run from pinned `rev`s in `.pre-commit-config.yaml` (prek-managed envs) so tool requirements can't constrain runtime pins. Python 3.11.
...
- `security.yml` jobs and their gates:
  ...
  - `osv` — official reusable workflow on `requirements.txt` only (`-L requirements.txt`), report-only. `dependency-review` (PRs) fails on new High+ deps.
- `release.yml` (push tag `v*`): `validate` (semver regex, tag must be ancestor of `origin/main`) → `build.yml` (reusable): `git archive` of the tag → syft SPDX+CycloneDX SBOMs of `requirements.txt` → `SHA256SUMS` → `cosign sign-blob` bundles → `actions/attest` v4 (provenance mode over `release-subjects/*`, then SBOM mode with `sbom-path`) → self-verify (`gh attestation verify`, `cosign verify-blob`) → `gh release create`. No Docker in releases; the service is deployed from source. Signing identity is anchored to `build.yml`'s path: `gh attestation verify <artifact> --owner schmitthub --signer-workflow schmitthub/prodsec-challenge/.github/workflows/build.yml`. `build` job needs `contents: write`, `id-token: write`, `attestations: write`, `artifact-metadata: write`.
