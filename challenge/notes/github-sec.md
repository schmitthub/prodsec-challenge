...
- Open code scanning alerts: 2, both CodeQL python: `py/full-ssrf` (critical, `../records_api/app/routes/webhooks.py`) and `py/sql-injection` (high, `../records_api/app/db.py`). Zero dismissed, zero fixed. **review**: Semgrep, Bandit, Gitleaks and osv-scanner categories show no open alerts even though the seeded findings exist. Confirm their SARIF uploads carry results, or the alerts are being suppressed by the baseline/diff logic.
