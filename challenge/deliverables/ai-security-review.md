...
- **Secrets.** Local version runs in the developer's own session, so nothing leaves the boundary it is already in; findings still never echo values, and an eval plants a marker in `config/dev.py` that must not appear in the report. Avoiding sending secrets to other models would require network egress only, scanning outbound requests for sensitive data you don't want out is effectively impossible. Prompt requests can be massive, trying to scan in flight would be infeasible, large scale WAFs only buffer 128kb or so for example.
