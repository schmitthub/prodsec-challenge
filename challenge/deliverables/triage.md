...
- auth.py options={"verify_exp": False} allows for infinite token validity with no way to revoke. 
...
- Dockerfile runs as root user, if you are going to use this image in prod it is a risk 
- main.py `repr(exc)` is leaking sensitive information to an attacker 
