...
- Principal is an exported type alias for Callable[..., object] | Public | None. Every mutable principal declaration uses ClassVar[Principal]; selecting a different value must not narrow the attribute's type. Mypy's explicitly enabled mutable-override rule catches the original VS Code report; strict mode alone did not. The shared mypy/Ruff script runs in the local lint hook and Test workflow, with positive/negative checker regression tests and uv.lock pins.
...
- Current verification after lint hardening: the shared mypy/Ruff script and its actual pre-commit lint hook pass; 99 full-suite tests passed against Postgres; all 11 Semgrep rule fixture suites and the full app/ contract gate pass. Checker regressions prove mypy mutable-override rejects the original narrowed principal declaration and accepts ClassVar[Principal]; Ruff rejects incomplete/Any signatures, blanket/stale ignores, and mutable class defaults. Typed PUBLIC declarations also have a Semgrep fixture. Earlier throwaway git repositories proved unapproved PUBLIC/missing-policy block, justified exception passes visibly, blanket suppression blocks.
