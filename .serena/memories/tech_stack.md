...
- `requirements.txt` is the canonical manifest: SBOM (syft `file:requirements.txt`) and all SCA (osv `-L requirements.txt`, dependency-review) read only it. Runtime pins (exact `==`): fastapi 0.128.0, uvicorn 0.38.0, PyJWT 2.12.0, requests 2.31.0, httpx 0.27.0 — mirrored in `pyproject.toml`/`uv.lock` (uv added by the user for dev ergonomics; CI checks the mirror)
