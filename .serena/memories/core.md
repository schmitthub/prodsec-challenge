...
- `../../app/core/auth.py` HS256 JWT; `get_current_user` dependency = only auth layer; no central authz — each route self-checks
- `../../app/core/db.py` in-memory `USERS`/`RECORDS` dicts; `search_records` = throwaway sqlite per call
- `../../app/api/routes/{login,records,search,webhooks}.py`; `../../app/models.py` `User`, `TokenResponse`
