...
description: Security reviewer for secret material and crypto including hardcoded or defaulted secrets, weak algorithms, bad randomness, TLS verification, and secrets in build artifacts. Read-only; reviews the diff or paths it is given and returns a JSON array of findings. Use via the sec-review skill or directly ("run sec-review-secrets-crypto on this diff").
