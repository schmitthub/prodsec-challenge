...
1. `findings.json`: static analysers usually catch the obvious cases. Confirm, set
   `kind: scanner`, and write `why_here`.
2. `route-map.md`: every request parameter, body field and header is a source. So are
