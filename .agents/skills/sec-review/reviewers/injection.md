...
description: Security reviewer for untrusted data reaching interpreters including SQL/NoSQL, command, code/template, path traversal, and header/log injection. Read-only; reviews the diff or paths it is given and returns a JSON array of findings. Use via the sec-review skill or directly ("run sec-review-injection on this diff").
